...
object WorkflowCostDAO {
  def apply[F[_]: Async]/*(transactor: Transactor[F])*/: WorkflowCostDAO[F] = new WorkflowCostDAO()
//
//  val createSql: Update0 =
//    (fr"CREATE TABLE IF NOT EXISTS" ++ workflowTableName ++
//      fr"(id SERIAL NOT NULL," ++
//      workflowIdFragment ++ fr"VARCHAR(255) NOT NULL," ++
//      parentWorkflowIdFragment ++ fr"VARCHAR(255)," ++
//      rootWorkflowIdFragment ++ fr"VARCHAR(255)," ++
//      workflowCollectionIdFragment ++ fr"VARCHAR(255) NOT NULL," ++
